                ...
                <div class="card-body">
                    <form action="{{ route('items.store') }}" method="POST" enctype="multipart/form-data">
                                ...
                                <div class="mb-3">
                                    ...
                                    @if ($categorias->count() == 0)
                                        <div class="form-text text-warning">
                                            ...
                                            <a href="{{ route('categorias.create') }}" class="text-decoration-none">Criar
                                                categoria</a>
                                        </div>
                                    @endif
                                </div>
                            ...
                            <div class="col-md-12">
                                <div class="mb-3">
                                    ...
                                    @error('imagem')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                    <div class="form-text">Selecione uma imagem para representar o item (opcional).</div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row">
                            <div class="col-md-4">
                                <div class="mb-3">
                                    ...
                                    <input type="number" class="form-control @error('quantidade') is-invalid @enderror"
                                        id="quantidade" name="quantidade" value="{{ old('quantidade', 0) }}" min="0"
                                        step="1" required>
                                    @error('quantidade')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                            ...
                            <div class="col-md-4">
                                <div class="mb-3">
                                    ...
                                    <select class="form-select @error('unidade') is-invalid @enderror" id="unidade"
                                        name="unidade" required>
                                        <option value="">Selecione a unidade</option>
                                        <option value="un" {{ old('unidade') == 'un' ? 'selected' : '' }}>Unidade (un)
                                        </option>
                                        <option value="kg" {{ old('unidade') == 'kg' ? 'selected' : '' }}>Quilograma
                                            (kg)</option>
                                        <option value="g" {{ old('unidade') == 'g' ? 'selected' : '' }}>Grama (g)
                                        </option>
                                        <option value="l" {{ old('unidade') == 'l' ? 'selected' : '' }}>Litro (l)
                                        </option>
                                        <option value="ml" {{ old('unidade') == 'ml' ? 'selected' : '' }}>Mililitro
                                            (ml)</option>
                                        <option value="m" {{ old('unidade') == 'm' ? 'selected' : '' }}>Metro (m)
                                        </option>
                                        <option value="cm" {{ old('unidade') == 'cm' ? 'selected' : '' }}>Centímetro
                                            (cm)</option>
                                        <option value="m²" {{ old('unidade') == 'm²' ? 'selected' : '' }}>Metro
                                            Quadrado (m²)</option>
                                        <option value="m³" {{ old('unidade') == 'm³' ? 'selected' : '' }}>Metro Cúbico
                                            (m³)</option>
                                        <option value="cx" {{ old('unidade') == 'cx' ? 'selected' : '' }}>Caixa (cx)
                                        </option>
                                        <option value="pct" {{ old('unidade') == 'pct' ? 'selected' : '' }}>Pacote (pct)
                                        </option>
                                        <option value="dz" {{ old('unidade') == 'dz' ? 'selected' : '' }}>Dúzia (dz)
                                        </option>
                                    </select>
                                    @error('unidade')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="number"
                                            ...
                                            value="{{ old('preco_unitario') }}" min="0" step="0.01"
                                            placeholder="0,00">
                                    </div>
                        ...
                        <div class="alert alert-info">
                            ...
                            <strong>Dica:</strong> Após criar o item, você poderá fazer movimentações de entrada e saída
                            através do menu "Movimentações".
                        </div>
